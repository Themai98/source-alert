...
uint64_t getRealOffset1(uint64_t offset){
	return KittyMemory::getAbsoluteAddress("anogs", offset);
}

// Patching a offset without switch.
void Anogs(uint64_t offset, std::string hexBytes) {
	MemoryPatch patch = MemoryPatch::createWithHex("anogs", offset, hexBytes);
	if(!patch.isValid()){
